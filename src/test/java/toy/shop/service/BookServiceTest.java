...
class BookServiceTest {
    ...
    void bookEnrollTest() throws Exception {
        ...
        Book book = new Book(dto.getBookName(),dto.getPubleYear(),dto.getPublisher(),dto.getBookPrice(), dto.getBookPrice(), dto.getBookDiscount(),dto.getBookIntro(),dto.getBookContents());
    ...
    void changeDtoValues() throws Exception {
        ...
        Book book = new Book("기존이름", LocalDate.now(), "기존퍼블리셔", 20000, 100, 0.1, "기존인트로", "기존컨텐츠");
        book.setCategory(null);
        book.setAuthor(null);
        //when
        bookService.bookEnroll(book);
        bookService.changeValues(book.getId(), dto, author, category);
        ...
        assertThat(findBook.getPubleYear()).isEqualTo(dto.getPubleYear());
    ...
    private Author initAuthor(final String authorName,AuthorNation nation,String intro) {
        AuthorDto author = new AuthorDto(authorName, nation,intro);
        Long savedId = authorService.authorEnroll(author);
        return authorService.findById(savedId).get();
    ...
    private static BookDto getBookDto(final Author author, final Category category) {
        return new BookDto("새로운 북이름", author.getId(), LocalDate.now(), "새로운 퍼블리셔", category.getCateCode(), 10000, 100, 0.2, "새로운 인트로", "새로운 콘텐츠");
    }
}